...
class MultipartParam(object):
    ...
    def __init__(self, name, value=None, filename=None, filetype=None,
        ...
        if filename is None:
            ...
            #        replace('"', '\\"')
            self.filename = str(self.filename, "utf-8")
            self.filename.encode("utf-8").decode('unicode_escape', 'ignore').replace('"', '\\"')
...
def multipart_encode(params, boundary=None, cb=None):
    ...
    if boundary is None:
        ...
        #nima
        boundary = urllib.parse.quote_plus(boundary)#urllib.parse.quote_plus(boundary)
